...
//   let picture: any[] = [];
  ...
  onMount(async () => {
    ...
    const res = await fetch("api/destaques/getAll", {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
    const data = await res.json();
    destaquesList = data.destaques;
    console.log(destaquesList);
});


  async function createDestaques(event: any) {
    ...
    console.log(picture);
    const res = await fetch("/api/destaques/create", {
      method: "POST",
      body: formData,
      headers: {
        Authorization: `Bearer ${token}`,
    ...
    if (res.ok) {
      const newDestaques = await res.json();
      console.log(newDestaques);
      destaquesList = [...destaquesList, newDestaques.destaque];
      destaques.name = "";
  ...
  async function deleteDestaques() {
    const res = await fetch("api/destaques/delete", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
      },
      body: JSON.stringify({ id:destaques.id }),
    });

    if (res.ok) {
      console.log("aaa");
      destaquesList = destaquesList.filter((p) => p.id !== destaques.id);
    }
  }
  ...
  function openModalConfirm(idParam:number){
    destaques.id = idParam;
    document.getElementById("my_modal_confirm")?.showModal();
  }

  function editDestaques(idParam:number){
            destaques.id = idParam;
            goto(`/destaquesAdmin/${destaques.id}`);
...
<div class="overflow-y-scroll h-screen">
  ...
  <!--Delete Modal-->
  <ModalConfirm message="Excluir" metodo={deleteDestaques} />
</div>
