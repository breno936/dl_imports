...
export const DELETE: RequestHandler = async ({ request }) => {
  try {
    ...
    const pictures = await prisma.pictures.findMany({
      where:{maisVendidosId:existingVendidos?.id}
    })
    ...
    await prisma.maisVendidos.delete({
      where: { id }
    });

    return new Response(JSON.stringify({ message: 'Register deleted' }), { status: 200 });
  // }else{
  ...
  // }
    ...
    return new Response(JSON.stringify({ error: 'Failed to delete register' }), { status: 500 });
  }
