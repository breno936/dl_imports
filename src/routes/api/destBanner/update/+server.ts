...
export const PUT: RequestHandler = async ({ request }) => {
    ...
    const existingBanner = await prisma.destBanner.findFirst({
      where:{id},
    });
      ...
      if(typeof picture == 'object'){
        ...
        deleteImage(existingBanner.picture);
        filePath = optimizedUrl;

      }


    const updateBanner = await prisma.destBanner.update({
      where:{id},
    ...
    return new Response(JSON.stringify({ banner: updateBanner }), { status: 201 });
