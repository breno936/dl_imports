...
export const DELETE: RequestHandler = async ({ request }) => {
  try {
    ...
    const pictures = await prisma.pictures.findMany({
      where:{novidadesId:existingNovidades?.id}
    })
    ...
    await prisma.novidades.delete({
      where: { id }
    });

    return new Response(JSON.stringify({ message: 'Novidade deleted' }), { status: 200 });
  // }else{
  ...
  // }
    ...
    return new Response(JSON.stringify({ error: 'Failed to delete novidade' }), { status: 500 });
  }
