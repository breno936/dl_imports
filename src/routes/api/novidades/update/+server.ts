...
export const PUT: RequestHandler = async ({ request }) => {
    ...
    const existingNovidades = await prisma.novidades.findFirst({
      where:{id},
      include:{
        size:true
      }
    ...
    const sizesToDisconnect = existingNovidades?.size.map(size => ({ id: size.id })) || [];

    
    const updateNovidades = await prisma.novidades.update({
      where:{id},
    ...
    updateNovidades.pictures = [];

    for (const element of picturesResult) {
      let pictures = await prisma.pictures.create({
      
        data:{novidadesId:updateNovidades.id, namePath:element},
      })
    };
    
    updateNovidades.pictures = await prisma.pictures.findMany({
      where:{
        novidadesId:updateNovidades.id
      }
    })

    return new Response(JSON.stringify({ novidade: updateNovidades }), { status: 201 });
