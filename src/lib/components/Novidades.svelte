              ...
              <div class="swiper-slide rounded-lg flex flex-col">
                ...
                <div class="w-full h-2/6 text-center description">
                  <div class="sizeSelect w-full hidden h-44 absolute -mt-44 z-40">
                    <select bind:value={n.uniqueSize} class="w-2/3 max-w-xs px-4 py-2 mt-3 rounded-xl">
                      <option value="" disabled selected>Tamanho</option>

                      {#each n.size as s(s.id)}
                  ...
                  <br/>
                  <button on:click={(event) => handleBuyButtonClick(event, n)} class="buttonBuyFinal font-medium mt-4 px-4 py-2 rounded-3xl bg-black text-white">COMPRAR</button>
                 </div>
                <a href="/productDetail/destaques/{n.id}" class="productName">

                  <h3 class="inline-block">{n.name}</h3>
